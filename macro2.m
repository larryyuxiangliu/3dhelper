...
filename = 'dirichilet_per_results.txt';
fid = fopen(filename, 'a');
tic;
%%%% =========== setup of the parameters ==================================
PPs = 35:2:35;
N0s = 11:1:13;     % N0 is related to the number of the RB expansion coefficients
for PP = PPs
    for N0 = N0s
        N1 = 200;    % the number of boundary pts on the object in the r-z plane;
        M1 = 1.2 * N1;    % the number of MFS pts in the r-z plane;
        P = 120;     % number of Fourier Modes;
        q = 320;
        M2 = 23;    % the points on the box, how many points along 1 direction on each surface; so totally 6*M2^2 points on the box;
        ...
        source = source3DQuad(f, f_t, N1, q, Rt, hs);      % curve MFS points, propotional to local speed;
        ...
        b = coeffBORto3D(Afbn, q);   % this is used to do the far-field evalution;
        toc;
        ...
        ff = @(x)  apply_GMRES_Matrix_BOR_SVD(target, AU, iA, AV, QU, iQ,QV, B, C, ...
            M1, P, q, xs, ys, zs, [],[],[],x, k, [], nei, alpha, beta, ex, ey, type);
        
        eta = gmres(ff, b, [], 1e-12, N_iter);
        ...
        if 1,
            ...
            f = @(s) 1+a*cos(w*(s-p));
            f_t = @(s) -a*w*sin(w*(s-p));
            error = getBdyError(M_test, P_test, f, f_t, Rt,xs,ys,zs, [],[],[], k,[], kx, ky, kz,  PP, eta, xi, nei ,alpha, beta, ex, ey, type);
        end
